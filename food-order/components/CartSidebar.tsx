...
import { X, Minus, Plus, ShoppingBag, Trash2, ChevronRight, UtensilsCrossed } from 'lucide-react'
                ...
                {items.length === 0 ? (
                  ...
                  >
                    <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center">
                      <UtensilsCrossed size={32} strokeWidth={1.5} className="text-orange-300" />
                    </div>
                  ...
                  items.map(ci => {
                    const portionInfo = PORTIONS[ci.portion]
                    ...
                    return (
                      ...
                      >
                        <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0">
                          <FoodImage src={ci.item.image} alt={ci.item.name} emoji={ci.item.emoji} size="sm" />
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-1">
                            <div className="min-w-0">
                              ...
                              <span className="inline-block mt-0.5 text-[10px] font-semibold text-orange-500 bg-orange-50 border border-orange-100 px-1.5 py-0.5 rounded-md">
                                {portionInfo.label}
                              </span>
                            </div>
                            <button type="button" onClick={() => remove(ci.cartKey)} aria-label="Xóa"
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden">
                              <button type="button" aria-label="Giảm" onClick={() => setQty(ci.cartKey, ci.quantity - 1)}
                              ...
                              <button type="button" aria-label="Tăng" onClick={() => setQty(ci.cartKey, ci.quantity + 1)}
